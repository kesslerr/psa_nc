...
The origin of the square root in @eq-castellanos2 compared to @eq-luking1 is not clear. However, it inflates the noise ceiling estimation. @warrens2016 (TODO: read), to which the r package *splithalfr* refers to, and @pronk2022, also use @eq-luking1 and states, that it the method holds only if $\sigma_1^{sh}$ and $\sigma_2^{sh}$ do not differ substantially, and if the lengths of the two halves do not differ significantly. @warrens2016 also introduces alternative adjustment factors.
